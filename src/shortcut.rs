...
//!
//! Target = `<root>\Claude Desktop Updater.exe` (stable path — shortcut
//! survives version bumps). IconLocation points at a real `Claude.exe` so the
//! Start Menu renders Claude's own icon. On update we rewrite the shortcut to
//! retarget the icon at the newest version's `Claude.exe`.
...
pub const SHORTCUT_NAME: &str = "Claude Desktop Updater.lnk";

/// Where `<StartMenu>\Programs\Claude Desktop Updater.lnk` resolves for a given install
