...
//! metadata we don't need for diagnostics.
//!
//! Layout produced:
//!   <install_root>/versions/<version>/Claude.exe
...
pub fn extract_app(
    ...
    let claude_exe = final_dir.join("Claude.exe");
    if !claude_exe.exists() {
        bail!(
            "extracted tree has no Claude.exe at {} — MSIX layout changed?",
            claude_exe.display()
        );
    }
