...
use crate::config::{CONFIG_FILENAME, LEGACY_UPDATER_EXE_NAME, UPDATER_EXE_NAME};
...
/// - `root/Claude Desktop Updater.exe` or legacy `root/claude-launcher.exe`
...
pub fn validate_uninstall_root(root: &Path) -> Result<()> {
    ...
    if !root.join(UPDATER_EXE_NAME).is_file() && !root.join(LEGACY_UPDATER_EXE_NAME).is_file() {
        bail!(
            "refusing to uninstall: {} missing {} or {}",
            root.display(),
            UPDATER_EXE_NAME,
